...
function newvisited($user, $date)
{
    ...
    $row = mysqli_fetch_assoc(mysqli_query($db, "SELECT visitarray FROM $dbtableinfo WHERE id='$id'"));
...
function removeremvisit()
{
    ...
    mysqli_query($db, "UPDATE $dbtableinfo SET remainingvisits = remainingvisits - 1 WHERE id='$id'");
}
...
function maxviewsperday()
{
    ...
    $row = mysqli_fetch_assoc(mysqli_query($db, "SELECT remainingvisits FROM $dbtableinfo WHERE id='$id'"));
...
function getmyreferid()
{
    ...
    $query = mysqli_query($db, "SELECT id FROM $dbtableinfo WHERE ip_address='$ip' AND acc_prior > -3 LIMIT 0,1");
...
function getmypoints($round = false, $texted = true, $commas = true, $dots = true)
{
    ...
    $query = mysqli_query($db, "SELECT points FROM $dbtableinfo WHERE ip_address='$ip'");
...
function setpoints($id, $pointsquantity, $type = 0)
{
    ...
    if (thisexists()) {
        mysqli_query($db, "UPDATE $dbtableinfo SET points = $allpoints WHERE id='$id'");
    } else {
...
function setexperience($id, $expquantity, $type = 0)
{
    ...
    if (thisexists()) {
        mysqli_query($db, "UPDATE $dbtableinfo SET exp = $allexp WHERE id='$id'");
    } else {
        die('Hubo un error al procesar la información. #0003');
    }
}
...
function recalculatelvl($id, $actualvl, $exp)
{
    ...
    mysqli_query($db, "UPDATE $dbtableinfo SET lvl = '$newlvl' WHERE id='$id'");

    $row = mysqli_fetch_assoc(mysqli_query($db, "SELECT multiplier FROM $dbtableinfo WHERE id='$id'"));;

    $actualmulty = $row['multiplier'];
    $newmulti = calculatemultfromlvl($newlvl);

    if ($newmulti > $actualmulty) {
        mysqli_query($db, "UPDATE $dbtableinfo SET multiplier = '$newmulti' WHERE id='$id'");
    }
    ...
    if($templvl % 5 === 0)
    {
        ...
        {
            $newmult = $templvl/5+1;
            mysqli_query($db, "UPDATE $dbtableinfo SET multiplier = '$newmult' WHERE id='$id'");
        }
    } else
    {
...
function isnew($post_type)
{
    ...
    $row = mysqli_fetch_assoc(mysqli_query($db, "SELECT notifications FROM $dbtableinfo WHERE id='$id'"));
